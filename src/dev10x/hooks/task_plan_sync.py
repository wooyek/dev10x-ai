...
def write_plan_atomic(*, plan_path: Path, plan_data: dict[str, Any]) -> None:
...
def ensure_plan_metadata(plan: dict[str, Any]) -> None:
    if "plan" not in plan:
        plan["plan"] = {
            "created_at": now_iso(),
            "branch": get_branch(),
...
def set_nested(*, d: dict[str, Any], dotpath: str, value: str) -> None:
    keys = dotpath.split(".")
    for key in keys[:-1]:
        d = d.setdefault(key, {})
    try:
