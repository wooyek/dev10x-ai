#!/usr/bin/env python3
"""PreToolUse hook: block shell-based file writes and eval patterns.

Blocks:
  1. cat >, cat <<, echo >, printf > — use Write/Edit tools instead
  2. eval "$(...)" in skill files — use source <(script.sh) instead

Note: GIT_SEQUENCE_EDITOR and other env-prefix git patterns are now
handled by block-alias-covered-commands.py (GH-200).
