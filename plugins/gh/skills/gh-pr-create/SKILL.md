...
- **For simple/trivial fixes** (flaky tests, typos, single-line changes):
  ...
  format without invoking the full `dx:jtbd` skill.
- **For features, bug fixes, and multi-commit PRs**: follow the `dx:jtbd`
  base skill workflow:
...
- **PR body starts with the Job Story** — sourced from ticket description,
  ticket comments, or generated fresh using the `dx:jtbd` base skill.
